...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, (hn,cn) = self.encoder(packed_seq)
        padded_seq = pad_packed_sequence(enc_hiddens)
        enc_hiddens = padded_seq[0].permute(1,0,2)
        
        hn = torch.cat((hn[0],hn[1]),1)        
        init_decoder_hidden = self.h_projection(hn)
        
        cn = torch.cat((cn[0],cn[1]),1)        
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        unsqueezed_alpha_t = torch.unsqueeze(alpha_t,1)
        a_t = torch.squeeze ( torch.bmm(unsqueezed_alpha_t,enc_hiddens), 1)
